<template>
  <div class="property-path-detailed-view p-6 max-w-7xl mx-auto">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
      <BarChart
        :title="'Violations per Subject'"
        :xAxisLabel="'Subjects'"
        :yAxisLabel="'Violations'"
        :data="violationsPerSubjectChart"/>
      <BarChart
      ...
      <BarChart
        :title="'Violations per Violation Type'"
        :xAxisLabel="'Violation Type'"
        :yAxisLabel="'Count'"
        :data="violationsByMessageCategoryChart"/>
    </div>
...
const showDefinition = ref(false); const totalOccurrences = ref(342);
...
const violationRate = computed(() =>
  totalOccurrences.value > 0 ? Math.round((violatingTriples.value / totalOccurrences.value) * 100) : 0
);
...
const triplesWithViolations = computed(() =>
  triples.value.filter(t => t.violations.length > 0)
);
